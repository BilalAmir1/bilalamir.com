                        ...
                        return (
                            <m.div
                                ...
                                className={`
                                    ...
                                    ${isExactMatch
                                        ? "bg-zinc-800/60 border-emerald-500/50 shadow-[0_0_30px_rgba(16,185,129,0.15)] scale-[1.02] z-10"
                                        : hasActiveSkill
                                            ? "bg-zinc-800/40 border-emerald-500/30 shadow-none scale-100"
                                            : "border-zinc-800/50 bg-zinc-900/20 hover:bg-zinc-900/40 hover:border-zinc-700/80"
                                    } 
                                    ${isDimmed ? "opacity-30 grayscale" : ""}
                                `}
                            >
                                <span className={`absolute top-4 left-4 w-4 h-4 border-t border-l rounded-tl-lg pointer-events-none transition-all duration-500 ${isExactMatch ? "border-emerald-500/50 opacity-100" : "border-zinc-700/60 opacity-0 group-hover:opacity-100"}`} />
                                <span className={`absolute top-4 right-4 w-4 h-4 border-t border-r rounded-tr-lg pointer-events-none transition-all duration-500 ${isExactMatch ? "border-emerald-500/50 opacity-100" : "border-zinc-700/60 opacity-0 group-hover:opacity-100"}`} />
                                <span className={`absolute bottom-4 left-4 w-4 h-4 border-b border-l rounded-bl-lg pointer-events-none transition-all duration-500 ${isExactMatch ? "border-emerald-500/50 opacity-100" : "border-zinc-700/60 opacity-0 group-hover:opacity-100"}`} />
                                <span className={`absolute bottom-4 right-4 w-4 h-4 border-b border-r rounded-br-lg pointer-events-none transition-all duration-500 ${isExactMatch ? "border-emerald-500/50 opacity-100" : "border-zinc-700/60 opacity-0 group-hover:opacity-100"}`} />

                                <div>
                                    <div className="flex justify-between items-start mb-6">
                                        <div className={`p-3 rounded-2xl transition-colors duration-500 ${isExactMatch ? "bg-emerald-500/20 text-emerald-400" : "bg-zinc-900 border border-zinc-800 text-zinc-400 group-hover:border-zinc-700 group-hover:text-zinc-300"}`}>
                                            <FolderGit2 className="w-6 h-6" />
                                        </div>
                                        <ArrowUpRight className={`w-5 h-5 transition-all duration-500 ${isExactMatch ? "text-emerald-400 translate-x-1 -translate-y-1" : "text-zinc-600 group-hover:text-emerald-400 group-hover:translate-x-1 group-hover:-translate-y-1"}`} />
                                    </div>

                                    <p className="text-emerald-400 font-mono text-xs uppercase tracking-wider mb-3">{project.role}</p>
                                    <h3 className={`text-2xl font-bold mb-4 transition-colors duration-500 ${isExactMatch ? "text-emerald-50" : "text-zinc-50"}`}>{project.title}</h3>
